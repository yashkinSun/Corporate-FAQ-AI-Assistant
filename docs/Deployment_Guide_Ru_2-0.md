...
PG_PORT=5432
# -- REDIS SETTINGS --REDIS_HOST=botvopros-redis
REDIS_PORT=6379

# --- Context Memory (optional) ---
# Включить/выключить контекстную память (true/false)
CONTEXT_MEMORY_ENABLED=true
# База данных Redis для контекстной памяти (отличная от rate limiting)
CONTEXT_MEMORY_REDIS_DB=1
# Время жизни контекста в днях
CONTEXT_MEMORY_TTL_DAYS=1
# Максимальное количество сообщений в контексте
CONTEXT_MEMORY_MAX_MESSAGES=10# -- Остальные параметры имеют разумные значения по умолчанию в конфиге и могут быть добавлены в .env при необходимости кастомных значений -- 
...
4.  **Проверьте работу контекстной памяти**:
    - Напишите боту несколько сообщений.
    - Затем задайте уточняющий вопрос, например, «расскажи подробнее».
    - Бот должен ответить, учитывая контекст предыдущего сообщения.
    - Отправьте команду `/clear`. Бот должен подтвердить очистку контекста.
    - Снова задайте уточняющий вопрос. Теперь бот не должен понять его, так как контекст очищен.
