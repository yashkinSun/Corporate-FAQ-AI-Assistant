...
**ККэширование и очереди: Redis 5.0.1 для rate limiting и контекстной памяти диалогов. Система использует разные базы данных Redis для разделения этих функций..
...
**Управление сессиями и эскалация.** Система отслеживает сессии пользователей в PostgreSQL, а контекст диалогов (историю сообщений) хранит в Redis для быстрого доступа. Это позволяет боту «помнить» предыдущие вопросы и ответы в рамках одной сессии. В PostgreSQL сохраняются метаданные сессий, уровни уверенности ответов и временные метки эскалации. При необходимости эскалации к оператору создается запрос в `bot/operator.py`, который уведомляет доступных операторов через Telegram и управляет передачей диалога.

**Оптимизация БД и удержание истории.** Для ускорения выборок по активным чатам, сессиям и follow-up вопросам в PostgreSQL добавлены индексы на ключевые поля (`user_id`, `session_id`, `status`, `message_id`). Они поставляются через Alembic миграцию `migrations/versions/add_filter_indexes.py` и сокращают задержку RAG-запросов и эскалаций без изменения логики. Дополнительно в слое хранения лимитируется объем недавней истории уверенности и сообщений (по умолчанию до 5 записей на пользователя), что позволяет быстро вычислять триггеры эскалации и снижает потребление памяти фоновыми обработчиками.
...
├── utils/                  # Утилиты
│   ├── __init│   ├── context_memory.py    # Управление контекстной памятью в Redis
138	│   ├── db_monitor.py      # Мониторинг базы данных
139	│   ├── followup_manager.py # Управление follow-up вопросами
140	│   ├── greeting_detector.py # Детектор приветствий   ├── input_sanitization.py # Санитизация ввода
...
В PostgreSQL фиксированы индексы на фильтрационные поля сессий, сообщений, follow-up и операторских чатов; их создание описано в Alembic миграции `migrations/versions/add_filter_indexes.py`. Индексация ускоряет выборки контекста для RAG и проверку cooldown’ов эскалации, снижая задержку ответов под нагрузкой. В `storage/database_unified.py` функции `get_recent_confidence_scores` и `get_recent_messages` извлекают ограниченное число последних записей (по умолчанию 5) для каждого пользователя. Это ограничение сохраняет поведение RAG-пайплайна и логики эскалации, одновременно уменьшая риск утечек памяти в долгоживущих процессах и объем данных, удерживаемых в оперативке при формировании контекста.
...
### Контекстная память и управление диалогом

Система реализует продвинутую контекстную память на базе Redis, что позволяет боту поддерживать осмысленный диалог, учитывая предыдущие сообщения пользователя и свои ответы. Эта функция включается/отключается через переменную окружения `CONTEXT_MEMORY_ENABLED`.

**Ключевые особенности:**

- **Хранение в Redis:** История сообщений (вопросы пользователя и ответы бота) для каждого пользователя хранится в отдельном ключе в Redis в виде списка JSON-объектов. Это обеспечивает быстрый доступ к контексту без нагрузки на основную базу данных PostgreSQL.
- **Автоматическая очистка (TTL):** Для каждого ключа с контекстом устанавливается время жизни (`CONTEXT_MEMORY_TTL_DAYS`), по истечении которого история автоматически удаляется. Это предотвращает бесконечное накопление данных.
- **Ограничение размера контекста:** Максимальное количество сообщений в истории для одного пользователя ограничено параметром `CONTEXT_MEMORY_MAX_MESSAGES`, что позволяет контролировать объем передаваемого в LLM контекста и расход токенов.
- **Команда `/clear`:** Пользователи могут вручную очистить свою историю диалога с помощью команды `/clear`, что позволяет начать разговор «с чистого листа».
- **Graceful Degradation:** В случае недоступности Redis, функция контекстной памяти отключается, но бот продолжает работать в штатном режиме, отвечая на каждый вопрос как на новый, без учета предыдущего диалога.
- **Переформулирование вопросов:** При обнаружении follow-up вопроса (например, «расскажи подробнее» или «а сколько это стоит?»), система автоматически переформулирует его, добавляя контекст из предыдущего диалога. Это позволяет RAG-системе находить более релевантную информацию, даже если сам follow-up вопрос не содержит достаточного количества ключевых слов.
...
**Парсер Markdown документов** `parse_markdown()` удаляет встроенный HTML (например, `<div>` или `<span>`), нормализует заголовки (`#`, `##` и т. д.), списки (`-`, `*`, `+`, `1.`) и конвертирует таблицы с разделителями `|` в плоский текстовый вид. Ссылки формата `[текст](url)` преобразуются в `текст (url)`. Табличные разделители и строки-разделители (`---`) отбрасываются, чтобы избежать попадания вспомогательной разметки в индекс.

**Рекомендации по разметке для Markdown**:

- Используйте иерархию заголовков (`#` → `##` → `###`) для сохранения структуры документа.
- Списки оформляйте с единым маркером (`- ` или `1.`) без вложенных блоков HTML; вложенные списки поддерживаются, но лучше держать их неглубокими для предсказуемой сегментации.
- Таблицы задавайте классическим Markdown-форматом с `|` и строкой-разделителем `---`; избегайте объединённых ячеек и длинных многострочных ячеек, которые сложнее читать после нормализации.

**Замечания по ограничениям.** Встроенный HTML (таблицы, стили) из Markdown удаляется при парсинге, поэтому для сложной вёрстки используйте чистый Markdown. Файлы размером свыше 10 МБ отклоняются на этапе загрузки согласно настройке `MAX_FILE_SIZE`.

**Универсальный интерфейс** `parse_document()` автоматически определяет тип документа по расширению файла и вызывает соответствующий парсер. Система поддерживает расширения .pdf, .docx, .xls, .xlsx, .md и .markdown, а также включает обработку неподдерживаемых форматов.
...
**Механизм валидации конфигурации** реализован на основе Pydantic и запускается при старте приложения. Обязательные переменные окружения (`TELEGRAM_BOT_TOKEN`, `OPENAI_API_KEY`, `PG_USER`, `PG_PASSWORD`, `PG_HOST`, `PG_DB`) должны быть заданы, иначе инициализация завершается с ошибкой конфигурации. Числовые параметры проходят строгие проверки диапазонов: положительные значения требуются для `PG_PORT`, `WEB_PORT`, `INDEX_SCHEDULE_HOURS`, лимитов rate limiting и размеров кэшей; `CONFIDENCE_THRESHOLD` ограничен диапазоном `[0, 1]`, а `RERANKING_MIN_SCORE` — `[0, 10]`. Для `ADMIN_IDS` применяется парсер, конвертирующий строковый список в массив целых чисел и выбрасывающий ошибку при некорректных значениях.

**Таймауты обращения к OpenAI** задаются параметром `OPENAI_REQUEST_TIMEOUT` (по умолчанию 30 секунд, допускаются только значения больше нуля). Таймаут используется клиентом OpenAI для всех вызовов генерации и переранжирования, предотвращая зависание запросов. Пример рабочих значений в `.env`:

```
OPENAI_REQUEST_TIMEOUT=45
TELEGRAM_RATE_LIMIT_SECONDS=5
CONFIDENCE_THRESHOLD=0.75
RERANKING_MIN_SCORE=4.5
```

При срабатывании таймаута возвращается безопасный ответ, а временное сообщение индикатора «думаю» закрывается в рамках обработчика ошибок, описанного в разделе [Обработка ошибок и логирование](#обработка-ошибок-и-логирование).
...
Переменная `ALLOWED_EXTENSIONS` определяет список разрешенных расширений файлов для загрузки. По умолчанию поддерживаются PDF, DOCX, XLS, XLSX, MD и MARKDOWN форматы. При обработке Markdown встроенный HTML игнорируется, а списки и таблицы приводятся к плоскому тексту, что упрощает индексацию и снижает риск инъекций разметки.
