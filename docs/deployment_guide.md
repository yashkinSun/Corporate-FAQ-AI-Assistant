...
### Простое развертывание (бот + веб + Postgres/Redis)

Docker Compose поднимает два основных сервиса (бот и веб) и необходимые зависимости Postgres и Redis.
...
Создайте файл `.env` в корневой директории проекта со следующими переменными. RUN_MODE задается для конкретного сервиса в `docker-compose.yml` (боту — `telegram`, вебу — `web`).
...
**Пример volume-монтажа из `docker-compose.yml`:**

Compose-файл монтирует каталоги данных и логов хоста внутрь контейнеров. При необходимости скорректируйте пути на свои:

```yaml
services:
  botvopros-telegram:
    volumes:
      - /home/ubuntu/apps/chatbot/Botvoprosrelease/data:/app/data
      - /home/ubuntu/apps/chatbot/Botvoprosrelease/logs:/app/logs
  botvopros-web:
    volumes:
      - /home/ubuntu/apps/chatbot/Botvoprosrelease/data:/app/data
      - /home/ubuntu/apps/chatbot/Botvoprosrelease/logs:/app/logs
...
```bash
# Сборка и запуск всех сервисов (бот, веб, Postgres, Redis)
docker-compose up -d botvopros-redis botvopros-postgres botvopros-telegram botvopros-web
...
После первого запуска таблицы базы данных создаются автоматически. Веб-интерфейс будет доступен по адресу `http://localhost:5000`, а телеграм-бот начнет обрабатывать сообщения автоматически.

**Дополнительные шаги после запуска контейнеров:**

1. Выполните миграции схемы базы данных внутри контейнера веб или бота:
   - Через Alembic: `docker-compose exec botvopros-web alembic upgrade head`
   - Через встроенную команду: `docker-compose exec botvopros-web python migrations.py upgrade`

2. Проверьте появление новых индексов и работоспособность health-check эндпоинтов:
   - Сравните список индексов в целевой базе данных или выполните выборочное `EXPLAIN` для ключевых запросов.
   - Убедитесь, что эндпоинт `http://localhost:5000/health` возвращает успешный статус.

3. Перед запуском миграций убедитесь, что ключевые переменные конфигурации заданы в окружении контейнеров:
   - `JWT_SECRET_KEY`
   - `OPENAI_API_KEY` (или параметры Azure OpenAI)
   - Таймауты и ограничения запросов (`REQUEST_TIMEOUT`, `OPENAI_TIMEOUT`, `CHUNK_UPLOAD_TIMEOUT` и др.)
...
docker-compose logs botvopros-telegram

# Проверка подключения к базе данных
docker-compose exec botvopros-postgres pg_isready -U ${PG_USER:-botvopros_user}
```
